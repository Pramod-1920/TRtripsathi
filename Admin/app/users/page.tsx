  ...
  useEffect(() => {
    ...
    async function loadUsers() {
      ...
      try {
        ...
        if (active) {
          const items = (response.data?.items ?? []) as Array<Partial<User>>;
          const userItems = items.filter(
            (item): item is User => item.role === 'user' && typeof item._id === 'string',
          );
          const pagination = response.data?.pagination;
          setUsers(userItems);
          setTotal(Number(pagination?.total ?? userItems.length));
